...
func _ready():
	...
	# These signals must exist on Player (as you added)
...
func _highlight_active_anchor() -> void:
	# Optional: scale the active one up so you notice it
...
func _can_manifest_now() -> bool:
	...
	# needs player facing + LOS to anchor point
...
# -----------------------------
# Cooldown + pacing influenced by sprint/danger
# -----------------------------
...
func _debug_update(delta: float) -> void:
	...
	# Optional: show state on a small third line (remove if you want)
	var state_line := ""
